...
-   Use Lagrange multipliers, take ``functional derivative'' with respect to $q_j$, and set it to zero, we have
    $$
...
-   This is the logarithm of an inverse-gamma density function:
    $$
    g(\tau^2) = \text{Inv-Gamma}\left(\tau^2 \middle| 3, \frac{1}{2} \sum_{j=1}^8 \E_{-\tau}\left(\left(\alpha_j-\mu\right)^2\right)\right).
    $$
...
-   The mean-field approximation is 
    ...
    \tau^2 & \sim \text{Inv-Gamma}(3, B_{\tau}).
    \end{align*}
-   Thus
    \begin{align*}
    \E_{-\alpha_j}(\tau^{-2}) & = \E_{-\mu}(\tau^{-2})= \frac{3}{B_{\tau}}\\
...
\begin{align*}
M_{\alpha_j} & = \frac{\frac{1}{\sigma_j^2} y_j+\E_{-\alpha_j}\left(\frac{1}{\tau^2}\right) \E_{-\alpha_j}(\mu)}{\frac{1}{\sigma_j^2}+\E_{-\alpha_j}\left(\frac{1}{\tau^2}\right)} = \frac{\frac{1}{\sigma_j^2} y_j+\frac{3}{B_{\tau}} M_{\mu}}{\frac{1}{\sigma_j^2}+\frac{3}{B_{\tau}}} = \frac{B_{\tau} y_j + 3\sigma_j^2 M_{\mu}}{3\sigma_j^2 + B_{\tau}}\\
S^2_{\alpha_j} & = \frac{1}{\frac{1}{\sigma_j^2}+\E_{-\alpha_j}\left(\frac{1}{\tau^2}\right)} = \frac{1}{\frac{1}{\sigma_j^2}+\frac{3}{B_{\tau}}} = \frac{B_{\tau}\sigma_j^2}{3\sigma_j^2 + B_{\tau}}\\
M_{\mu} & = \frac{1}{8} \sum_{j=1}^8 \E_{-\mu}(\alpha_j) = \frac{1}{8} \sum_{j=1}^8 M_{\alpha_j}\\
S^2_{\mu} & = \frac{1}{8} \frac{1}{\E_{-\mu}\left(\frac{1}{\tau^2}\right)} = \frac{1}{8} \frac{1}{\frac{3}{B_{\tau}}} = \frac{B_{\tau}}{24}\\
...
sig2 <- c(14.9, 10.2, 16.3, 11.0, 9.4, 11.4, 10.4, 17.6)
...
for (i in 2:max_iter){
    for(j in 1:8){
        var_par$M_alpha[i, j] <- (var_par$B_tau[i-1] * y[j] + 3 * sig2[j] * 
                                      var_par$M_mu[i-1]) / 
            (3 * sig2[j] + var_par$B_tau[i-1])
        var_par$S2_alpha[i, j] <- var_par$B_tau[i-1] * sig2[j] / 
            (3 * sig2[j] + var_par$B_tau[i-1])
    }
    var_par$M_mu[i] <- mean(var_par$M_alpha[i, ])
    var_par$S2_mu[i] <- var_par$B_tau[i-1] / 24
...
plot(1:max_iter, var_par$S2_mu, type = "l", ylim = c(0,10), 
     xlab = "iteration", ylab = expression(S[mu]^2))
plot(1:max_iter, var_par$B_tau, type = "l", ylim = c(0,100), 
